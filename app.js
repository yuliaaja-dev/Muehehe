...
/* ==========================================================
   Part : 5I (Update)
   Notifikasi & Panduan Install PWA (Custom CSS,
   Selalu Tampil - Tidak Bergantung Pada Event Native)
...
const installInstructionsModal =
    document.getElementById("installInstructionsModal");
const installStepsList = document.getElementById("installStepsList");
const closeInstallInstructionsBtn =
    document.getElementById("closeInstallInstructionsBtn");
...
/* ==========================
   Deteksi Platform / Browser
   (Untuk Menentukan Isi Panduan)
========================== */
...
function detectPlatform(){
    ...
    const isChrome =
        /chrome|crios/i.test(ua) && !isEdge;
    const isSafari =
        /safari/i.test(ua) && !isChrome && !isEdge && !isFirefox;
...
/* ==========================
   Susun Langkah Instalasi Manual
   Sesuai Platform Yang Terdeteksi
========================== */
...
function getManualInstallSteps(){
    ...
    if(p.isIOS){
        ...
        return [
            "Tap ikon Share (kotak dengan panah ke atas) di bar bawah/atas Safari.",
            "Scroll ke bawah lalu pilih \"Add to Home Screen\".",
            "Tap \"Add\" di pojok kanan atas untuk konfirmasi."
        ];
    ...
    if(p.isAndroid){
        ...
        return [
            "Tap ikon menu titik tiga (⋮) di pojok kanan atas browser.",
            "Pilih \"Install app\" atau \"Add to Home screen\".",
            "Tap \"Install\" pada dialog konfirmasi yang muncul."
        ];

    }

    if(p.isDesktop && (p.isChrome || p.isEdge)){

        return [
            "Lihat ikon install (biasanya bentuk ⊕ atau layar kecil) di address bar sebelah kanan.",
            "Klik ikon tersebut, atau buka menu (⋮) lalu pilih \"Install Pong PWA...\".",
    ...
    if(p.isFirefox){
        ...
        return [
            "Firefox belum mendukung instalasi PWA satu-klik di semua platform.",
            "Di Android: buka menu (⋮) lalu cari \"Install\" atau \"Add to Home screen\".",
            "Di desktop: kamu tetap bisa main langsung tanpa install, cukup buka lewat browser."
        ];

    }

    return [
        "Buka menu browser kamu (biasanya ikon ⋮ atau ☰).",
        "Cari opsi \"Install App\" atau \"Add to Home Screen\".",
        "Ikuti instruksi yang muncul untuk memasang aplikasi."
    ];
...
function showInstallInstructions(){
    ...
    const steps = getManualInstallSteps();
    ...
    steps.forEach((stepText)=>{
        ...
        installStepsList.appendChild(li);

    });

    installInstructionsModal.classList.remove("hidden");
...
closeInstallInstructionsBtn.addEventListener("click",()=>{
...
/* ==========================
   ...
   -> Native Prompt Jika Ada,
   -> Panduan Manual Jika Tidak
========================== */
...
/* ==========================
   Tampilkan Banner Setelah
   Halaman Siap
   (Tidak Menunggu Event Native
   Apa Pun - Selalu Muncul)
========================== */
